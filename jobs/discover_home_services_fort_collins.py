#!/usr/bin/env python3
"""
USP Phase 1 — Discovery: home_services x Fort Collins, CO
EXPANSION CITY: Fort Collins
...
FORT_COLLINS_LOCATION = {"lat": 40.5853, "lng": -105.0844}

_HOME_SERVICE_QUERIES = [
    "roofing companies Fort Collins Colorado",
    "HVAC companies Fort Collins Colorado",
    "plumbing services Fort Collins Colorado",
]
...
def _log(action, detail=None, result=None):
    ...
    conn.execute(
        ...
        (action, "discovery_home_services_fortcollins", detail, result, _now_iso())
    )
    conn.commit()
    conn.close()


def run_discovery_home_services_fort_collins(radius_meters=12000, dry_run=False):
    cfg = _load_config()
    ...
    for query in _HOME_SERVICE_QUERIES:
        _log("discovery_query_start", detail=f"[home_services/FortCollins] {query}")
        try:
            raw = _places_search(query, FORT_COLLINS_LOCATION, radius_meters, api_key)
        ...
        conn.execute(
            """INSERT INTO discovery_places (query, location_lat, location_lng, radius_meters, raw_response, places_found, run_at)
               VALUES (?, ?, ?, ?, ?, ?, ?)""",
            (query, FORT_COLLINS_LOCATION["lat"], FORT_COLLINS_LOCATION["lng"], radius_meters,
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="USP Home Services Discovery — Fort Collins, CO")
    parser.add_argument("--dry-run", action="store_true")
    args = parser.parse_args()
    print(f"[{datetime.now(timezone.utc).isoformat()}] Starting home_services x Fort Collins, CO")
    result = run_discovery_home_services_fort_collins(radius_meters=12000, dry_run=args.dry_run)
