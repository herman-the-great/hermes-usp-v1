#!/usr/bin/env python3
"""
USP Phase 1 — Discovery: home_services x Boulder, CO
EXPANSION CITY: Boulder
Scope: 3 queries, 20 results each, Boulder city center.
Dedupe: against all existing place_ids in usp.db.
Does NOT generate packets or Gmail drafts.

NOTE: Disabled by default. Enable by adding to usp_daily_discovery.py run_script() calls.
Currently 3 queries x 5 cities = 15 queries/day = ~$0.48/day.
With all 5 Colorado cities enabled, becomes 18 queries/day = ~$0.58/day.
"""
import json
import os
import sys
...
# 3 queries per city = ~60 leads/day/city max
_HOME_SERVICE_QUERIES = [
    "roofing companies Boulder Colorado",
    "HVAC companies Boulder Colorado",
    "plumbing services Boulder Colorado",
]
...
def _log(action, detail=None, result=None):
    ...
    conn.execute(
        ...
        (action, "discovery_home_services_boulder", detail, result, _now_iso())
    )
    conn.commit()
    conn.close()


def run_discovery_home_services_boulder(radius_meters=12000, dry_run=False):
    """
    Discover home_services leads in Boulder, CO area.
    ...
    for query in _HOME_SERVICE_QUERIES:
        _log("discovery_query_start", detail=f"[home_services/Boulder] {query}")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="USP Home Services Discovery — Boulder, CO")
    parser.add_argument("--dry-run", action="store_true", help="Fetch but don't write to DB")
    args = parser.parse_args()

    print(f"[{datetime.now(timezone.utc).isoformat()}] Starting home_services x Boulder, CO discovery")
    print(f"Queries: {_HOME_SERVICE_QUERIES}")

    result = run_discovery_home_services_boulder(radius_meters=12000, dry_run=args.dry_run)
